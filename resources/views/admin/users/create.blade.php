...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.store") }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('graduacao') ? 'is-invalid' : '' }}" name="graduacao_id" id="graduacao_id" required>
                        @foreach($graduacaos as $id => $entry)
                            <option value="{{ $id }}" {{ old('graduacao_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group">
                <label class="required" for="cpf">{{ trans('cruds.user.fields.cpf') }}</label>
                <input class="form-control {{ $errors->has('cpf') ? 'is-invalid' : '' }}" type="text" name="cpf" id="cpf" value="{{ old('cpf', '') }}" required>
                @if($errors->has('cpf'))
                    <div class="invalid-feedback">
                        {{ $errors->first('cpf') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nomemae') ? 'is-invalid' : '' }}" type="text" name="nomemae" id="nomemae" value="{{ old('nomemae', '') }}">
            ...
            <div class="form-group">
                @can('io_pessoa_create')
                    <button class="btn btn-primary" type="submit" value="iopessoa" name="acao">
                        <i class="fa-fw fas fa-exchange-alt"></i>
                        {{ trans('global.saveio') }}
                    </button>
                @endcan
                @can('user_access')
                    <button class="btn btn-success" type="submit" value="voltar" name="acao">
